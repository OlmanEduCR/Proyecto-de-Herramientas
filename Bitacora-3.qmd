...
P(Y=1/X)=1/(1+e^−(β_0+β_1*X_1+⋯+β_p*X_p))
Donde:
...
El PCA transforma las variables originales en componentes ortogonales que maximizan la varianza en donde W es la matriz de autovectores de la matriz de covarianza de X
...
estadisticas_descrip <- data.frame(
  Variable = c("Nota de Admisión", "Edad", "Calificación Previa", "Nota 1er Semestre", "Nota 2do Semestre"),
  ...
  Cantida_NA = sapply(columnas_seleccionadas, function(x) sum(is.na(x)))
)
...
for(i in 1:ncol(columnas_seleccionadas)) {
  hist(columnas_seleccionadas[[i]], 
       main = estadisticas_descrip$Variable[i],
       xlab = "Valor",
       ylab = "Frecuencia",
       col = "lightblue",
       border = "black")
