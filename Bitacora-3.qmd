...
Para facilitar la revisión de la bitácora 3, se adjunta los paquetes que se tienen que cargar, además de la carga del archivo
...
Permite recolectar, organizar, analizar y presentar conjuntos de datos para identificar y comprender sus características principales, patrones y tendencias.
...
#Se imprimió cuando se estaba analizando para tener mejor comprensión
```
...
**analisis** 
En el análisis estadístico nos ayuda a poder ver las
...
**Gráficos de las variables que tienen relación directa con Target:**
...
datos %>%
  ...
  labs(
    title = "Créditos 1er Semestre: Matriculados y Aprobados según Estado Académico",
    x = "Estado académico",
...
datos %>%
  ...
  labs(
    title = "Créditos 2do Semestre: Matriculados y Aprobados según Estado Académico",
    x = "Estado académico",
    y = "Número de créditos",
    fill = ""
  )
...
datos %>%
  mutate(
    ...
    Marital.status = factor(
      ...
      labels = c("Soltero", "Casado", "Viudo", "Divorciado", "Unión Lib", "Separado")
    )
  ) %>%
  ...
  ggplot(aes(x = Marital.status, y = prop, fill = Target)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.9) +
  geom_text(
    aes(label = paste0(round(prop * 100, 1), "%")),
    position = position_stack(vjust = 0.5),
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Distribución de Deserción por Estado Civil",
    x = "Estado Civil",
    y = "Proporción",
    fill = "Estado"
  ) +
...
datos %>%
  mutate(
    Target = factor(Target,
                    levels = c("Graduate", "Enrolled", "Dropout"),
                    labels = c("No Desertor", "No Desertor", "Desertor")),
    Educational.special.needs = factor(Educational.special.needs, levels = c(0, 1), 
                                      labels = c("No", "Sí"))
  ) %>%
  ...
  ggplot(aes(x = Educational.special.needs, y = prop, fill = Target)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.9) +
  geom_text(
    aes(label = paste0(round(prop * 100, 1), "%")),
    position = position_stack(vjust = 0.5),
  ...
  labs(
    title = "Distribución de Deserción por Necesidades Educativas Especiales",
...
datos %>%
  mutate(
    Target = factor(Target,
                    levels = c("Graduate", "Enrolled", "Dropout"),
                    labels = c("No Desertor", "No Desertor", "Desertor")),
    Debtor = factor(Debtor, levels = c(0, 1), labels = c("No Deudor", "Deudor"))
  ) %>%
  # Agrupar para proporciones
  ...
  ggplot(aes(x = Debtor, y = prop, fill = Target)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.9) +
  geom_text(
    aes(label = paste0(round(prop * 100, 1), "%")),
    position = position_stack(vjust = 0.5),
  ...
  labs(
    title = "Distribución de Deserción por Condición de Deudor",
    x = "Condición de Deudor",
    y = "Proporción",
    fill = "Estado"
  )
```
...
datos %>%
  mutate(
    Target = factor(Target,
                    levels = c("Graduate", "Enrolled", "Dropout"),
                    labels = c("No Desertor", "No Desertor", "Desertor")),
    Scholarship.holder = factor(Scholarship.holder, levels = c(0, 1), 
                               labels = c("No Becario", "Becario"))
  ) %>%
  # Agrupar para proporciones
  ...
  ggplot(aes(x = Scholarship.holder, y = prop, fill = Target)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.9) +
  geom_text(
    aes(label = paste0(round(prop * 100, 1), "%")),
    position = position_stack(vjust = 0.5),
  ...
  labs(
    title = "Distribución de Deserción por Condición de Becario",
    x = "Condición de Becario",
    y = "Proporción",
    fill = "Estado"
  )
```
...
datos %>%
  ...
  ggplot(aes(x = International, y = prop, fill = Target)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.9) +
  geom_text(
    aes(label = paste0(round(prop * 100, 1), "%")),
    position = position_stack(vjust = 0.5),
  ...
  labs(
    title = "Distribución de Deserción por Condición de Estudiante Internacional",
